...
export default function EditAccountMovePage({ params }) {
    ...
    React.useLayoutEffect(() => {
        var config = {
            method: 'get',
            url: process.env.NEXT_PUBLIC_SERVER_URL + '/account-move/getAll',
            headers: {
        ...
        axios(config)
            .then((response) => {
                ...
                const recordDataProcessed = {
                    'id': recordData.id,
                    'kasir': recordData.kasir.id,
                    'source': recordData.source.id,
    ...
    async function handleSaveButtonClick(formData) {
        ...
        var config = {
            method: 'post',
            url: serverUrl + '/account-move/write',
            headers: {
    ...
    return (
        <AccountMovePageForm isEditPage={true} handleSaveButtonClick={handleSaveButtonClick} defaults={defaults} />
    )
}